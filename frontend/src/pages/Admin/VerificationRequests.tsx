  ...
  const handleAccept = async () => {
    if (selectedRequestId !== null) {
      try {
        ...
        setRequests(
          requests.map((req) =>
            req.id === selectedRequestId ? { ...req, status: 'ACCEPTED' } : req
          )
        );
      } catch (error) {
        console.log(error);
    ...
    <PageLayout title='Player verification requests'>
      <Link to={`/administration`}>
        <Button size='small' color='secondary'>
          <KeyboardBackspaceIcon sx={{ marginRight: '4px', marginY: '16px' }} /> Back
        </Button>
      </Link>
            ...
            >
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <Button
                  variant='contained'
                  color='success'
                  onClick={() => handleOpenDialog(request.id ?? 0, 'ACCEPT')}
                >
                  Accept
                </Button>
                <Button
                  variant='contained'
                  color='error'
                  onClick={() => handleOpenDialog(request.id ?? 0, 'REJECT')}
                >
                  Reject
                </Button>
              </div>
      ...
      >
        <DialogTitle id='alert-dialog-title'>
          {`Are you sure you want to ${confirmAction ? confirmAction.toLowerCase() : ''} this request?`}
        </DialogTitle>
        <DialogContent>
