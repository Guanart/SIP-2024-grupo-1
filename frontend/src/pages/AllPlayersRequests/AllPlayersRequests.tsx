...
export const AllPlayersRequests = () => {
  ...
  const handleAccept = async (id: number) => {
    try {
      await axios.patch(`http://${HOST}:${PORT}/verification-request/${id}`, { id, status: 'ACCEPTED' }, {
        headers: {
  ...
  const handleReject = async (id: number) => {
    try {
      await axios.patch(`http://${HOST}:${PORT}/verification-request/${id}`, { id, status: 'REJECTED' }, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
  ...
  return (
    <PageLayout title='All Player Requests'>
        <List>
